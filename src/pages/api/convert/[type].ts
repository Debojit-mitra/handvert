...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
    ...
    try {
        ...
        const originalName = uploadedFile.originalFilename || `upload-${Date.now()}`;
...
async function handleImageConvert(res: NextApiResponse, fields: Fields, inputPath: string) {
    ...
    if (dimensionType === '%' && (width || height)) {
        ...
        if (width && metadata.width) width = Math.round((width / 100) * metadata.width);
        if (height && metadata.height) height = Math.round((height / 100) * metadata.height);
    }
...
async function handleOfficeToPdf(res: NextApiResponse, inputPath: string, originalName: string) {
